...
namespace CrossingLears
{
    public class NamedPool<T> : MonoBehaviour where T : Component
    {
        ...
        {
            if (pools.TryGetValue(key, out Queue<T> queue) && queue.Count > 0)
            {
                T obj = queue.Dequeue();
                obj.gameObject.SetActive(true);
                return obj;
            }
            else if (TryGetOriginal(key, out T original))
            {
        ...
        {
            obj.close();
            if (!Instance.pools.TryGetValue(key, out Queue<T> queue))
            {
                queue = new Queue<T>();
